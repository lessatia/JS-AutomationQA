const { addStep, addFeature, addAttachment, addIssue, addArgument, addDescription, addEnvironment } =
...
exports.config = {
  ...
  //     allure.createAttachment("Screenshot мяв ", Buffer.from(image, "base64"), "image/png");
  //   });
  // },
  ...
  afterStep: async function (step, scenario, result, context) {
    ...
    //   const screenShotData = new Buffer.from(browser.takeScreenshot(), "base64");
    //   addAttachment("Screenshot xе", screenShotData, "image/png");
    // }
  },
  ...
  //         `./screenshots/${test.title
  //           .replace(/\s/g, "_")
  //           .toLowerCase()}_${new Date().getDate()}.${new Date().getMonth()}.${new Date().getFullYear()}_${new Date().getHours()}.${new Date().getMinutes()}.${new Date().getSeconds()}.png`
  //       );
  //     }
  //   },
  ...
  afterScenario: async (world, result, context) => {
    ...
    if (!result.passed) {
      addDescription('TESTTESTTEST!!!<img src="https://www.meme-arsenal.com/memes/f3136fbc5cc7d241bdaf3b317c416c70.jpg">');
      const screenShotData = new Buffer(await browser.takeScreenshot(), "base64");
      addAttachment("Sreenshot test", screenShotData, "image/png");
      const cookiesData = await browser.getAllCookies();
      addAttachment("Cookies test", cookiesData, "text/plain");
      const htmlData = await browser.getHTML();
      addAttachment("HTML test", htmlData, "text/html");
    }
